...
const EdiotUnitModal = ({
  ...
  const onSubmit = async (data) => {
    try {
      const result = await axiosInstance.patch(`units/${editableData._id}`, data)

      if (result.data.success) {
        setEditableData(data)
        setIsEditModalVisible(false)
        toast.success('Unit is updated successfully')
      }
    } catch (error) {
