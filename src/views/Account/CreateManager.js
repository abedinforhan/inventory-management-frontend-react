...
const CreateManger = () => {
  ...
  const { isLoading, isError, data: lastUserData } = useLastUserID('manager')

  const onSuccess = () => {
    queryClient.invalidateQueries('last-user-id')
  ...
  const {
    ...
    values: {
      id: 'M' + (parseInt(lastUserData?.data?.data) + 1).toString().padStart(5, '0'),
      gender: null,
    },
  })
